...
public class GhostAmbushBehavior extends Ghost.GhostBehavior {
	...
	public Direction getDirection() {
		...
		PathFinder.PathFindingResult toPacMan = PathFinder.pathFind(game.getLevelMap(), tileX, tileY, x, y,
				ghost.getPrevTile(), game.getPacMan().getPrevTile());
		if (toPacMan.distance < 6)
			return toPacMan.direction;
		...
		PathFinder.PathFindingResult toAmbush = PathFinder.pathFind(game.getLevelMap(), tileX, tileY, x, y,
				ghost.getPrevTile(), game.getPacMan().getPrevTile());
		if (toAmbush.distance == 0)
			return Utils.opposite(game.getPacManDirection());
